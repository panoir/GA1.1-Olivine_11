Hi I am Sohan, I just added this let me chek if its really added !!
function 'multiply' multiplies two numbers with each other, and returns the multiplication. You fill in two numbers for a and b, and the function returns the multiplications for a and b. To test it, you can choose numbers for a and b, and print the result.


The function "subtract" gets two numbers a and b and returns their difference a-b
The function "distance_from_zero(x)" gets the absolute value of the number which is the magnitude to distance of the number from zero.

The function "add(a, b)", has two numbers and returns the solution of a + b. So for example, you put add(5, 3), you will get 8 as a result.  

function "sum_list" gets a list of numbers and returns their sum through calling the "add" function previously created

Function square(x) is calling function "multiply". When you print function square(3) for example, the function will do multiply(3, 3), and you will get 9 as a result. 




